...
export function registerSkillImportCommand(skillCmd) {
    skillCmd
        .command("import")
        ...
        try {
            ...
            if (stat.isFile() &&
                ...
                formData.append("files[]", new Blob([buffer]), path.basename(resolvedPath));
            }
                ...
                spinner.stop();
                console.error(chalk.red("Error: Unsupported path. Provide a .zip file or directory."));
                process.exit(1);
            }
            const result = await client.postMultipart("/api/skills/import", formData);
            spinner.stop();
            console.log(chalk.green(`✓ Skill "${result.name}" imported successfully`));
        }
        catch (err) {
            spinner.stop();
