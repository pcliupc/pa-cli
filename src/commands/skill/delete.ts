...
export function registerSkillDeleteCommand(skillCmd: Command): void {
  skillCmd
    .command("delete")
    ...
    .action(async (skillName: string, options: { force?: boolean }) => {
      ...
      try {
        const skills = await client.getJson<SkillListItem[]>("/api/skills");
        ...
        try {
          ...
          if (msg.includes("referenced") || msg.includes("引用")) {
            console.error(
              chalk.red(
                `Error: Skill "${skillName}" is referenced by agents and cannot be deleted.`,
              ),
            );
          } else {
